...
- **学习节奏规则**：每个学习单元按「读原文 → 生成笔记 → 用户消化 → 用户提问/确认 → 对话补充 → review 笔记 → 进入下一阶段」循环。生成笔记后，必须等用户读完并确认（或提出问题），不要让用户马上进入下一阶段。笔记写完了不等于用户学完了
- **笔记 review 规则**：每个学习单元结束前（用户确认消化完毕），必须对照原文和对话内容完整 review 一次笔记，看有没有遗漏、有没有不准确、有没有需要补的地方。review 完毕后主动告知用户调整了什么
- **优先看文档**：用户问"下一步做什么"或"进度到哪了"时，优先引导看 `mcp-learning-outline.md`（学习大纲）或 `db-agent-plan.md`（项目方案），而不是凭记忆直接回答。让文档说话，不要让 Claude 的记忆说话
